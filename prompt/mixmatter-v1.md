...
Mixmatter may selectively retain text that already exists in the source when it contributes to scene identity. Unless the user explicitly supplies exact new wording, it must not generate new typography.
...
Mixmatter may crop, partially obscure, layer, or retain the original typography as source imagery, but it must not redesign the source into a new typography system.
...
Mixmatter editoriality comes from composition, cropping, hierarchy, image-state contrast, texture, print treatment, and visual rhythm.
...
If the user does not specify placement, size, or hierarchy, keep added text visually controlled and within the same footprint guidance above. Explicitly requested text does not permit unrelated copy or turn Mixmatter into a from-scratch typography generator.
